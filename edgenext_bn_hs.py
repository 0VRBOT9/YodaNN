...
class EdgeNeXtBNHS(nn.Module):
    def __init__(self, in_chans=3, num_classes=10,  # Adjusted for CIFAR-10
                 depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
                 global_block=[0, 0, 0, 3], global_block_type=['None', 'None', 'None', 'SDTA_BN_HS'],
                 drop_path_rate=0.1,  # Adjusted drop path rate
                 layer_scale_init_value=1e-6, head_init_scale=1., expan_ratio=4,
                 kernel_sizes=[3, 3, 3, 3],  # Adjusted kernel sizes for CIFAR-10
                 heads=[8, 8, 8, 8], use_pos_embd_xca=[False, False, False, False],
        ...
        self.head_dropout = nn.Dropout(kwargs.get("classifier_dropout", 0.5))  # Adjusted dropout rate
